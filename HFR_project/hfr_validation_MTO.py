...
#            '1M', date_in, date_fin, ds_restricted)
        ...
        if not start_bool_value or not final_bool_value:
            ...
            continue
        max_hfr_original = np.nanmax((ds_restricted['EWCT'][idx1_original:idx2_original+1, 0].data **
                                     2 + ds_restricted['NSCT'][idx1_original:idx2_original+1, 0].data ** 2) ** 0.5)
        min_hfr_original = np.nanmin((ds_restricted['EWCT'][idx1_original:idx2_original+1, 0].data **
                                     2 + ds_restricted['NSCT'][idx1_original:idx2_original+1, 0].data ** 2) ** 0.5)
...
#            time_res_to_average, date_in, date_fin, averaged_ds)
        ...
        for exp in range(len(name_exp_arr)):
            ...
            for time_counter, index in enumerate(range(idx1, idx2+1)):
                ...
                speed_hfr = np.ma.filled(speed_hfr, np.nan)
                ...
                if not isinstance(spatial_mean_model_ts_instant, float):
                    spatial_mean_model_ts_instant = np.nan
            ...
            if exp == 0:
                ...
                print("len_not_nan_values: ", len_not_nan_values)
        ...
        a = a[~np.isnan(b)]
        b = b[~np.isnan(b)]
        print("massimo di b: ", np.max(b[~np.isnan(a)]))
        print("maximum_value: ", maximum_value)
        ...
        for exp in range(len(name_exp_arr)):
            ...
            mean_vel_mod = round(np.mean(a[~np.isnan(a)]), 2)
            mean_vel_obs = round(np.mean(b[~np.isnan(a)]), 2)
            tot_mean_stat = [mean_vel_mod, mean_vel_obs]
    ...
    for exp in range(len(label_plot_arr)):
        print("prova dict: ", mod_array_qq[exp])
        ...
        title = 'Mean surface Current Velocity -ALL \n Period: ' + date_in + '-' + date_fin
        xlabel = 'Observation Current Velocity [m/s]'
        ylabel = 'Model Current Velocity [m/s]'
        if timerange.shape[0] > 2:
        ...
        a_file = open(path_to_out_plot_folder_arr[exp]+"statistics_areal_info_" +
        ...
        print("statistics: ", statistics_areal_info)
        ...
        print("unlisted array: ", unlisted_array)
        print(unlisted_array.shape)
        print("obs array qq: ", obs_array_qq)
        print(obs_array_qq.shape)
        tot_mean_mod = round(np.nanmean(unlisted_array), 2)
        ...
        a_file = open(path_to_out_plot_folder_arr[exp]+"statistics_qq_plots_" +
                      name_exp_arr[exp] + "_" + date_in + "_" + date_fin + ".csv", "w")
        writer = csv.writer(a_file)
        writer.writerow(["name_hfr", "mean_mod", "mean_obs", "bias",
                        "rmse", "si", "corr", "stderr", "number_of_points"])
        print("statistics: ", statistics_qq)
