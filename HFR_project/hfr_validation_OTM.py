...
def main(args):
    ...
    statistics_rev_interp_areal_info = {}
    statistics_qq_rev = {}
    spatial_not_interp_mean_model_ts = {}
    len_not_interp_model = {}
...
#            time_res_to_average, date_in, date_fin, averaged_ds)
        ...
        statistics_rev_interp_areal_info[ds.id] = {}
        statistics_qq_rev[ds.id] = {}
        statistics_rev_interp_areal_info["ALL HFR STATIONS"] = {}
        statistics_qq_rev["ALL HFR STATIONS"] = {}
        spatial_mean_interp_hfr_ts[ds.id] = {}

        len_not_interp_model[ds.id] = {}
        len_interp_hfr[ds.id] = {}
        ...
        for exp in range(len(name_exp_arr)):
            ...
            #statistics_rev_interp_areal_info[ds.id][exp] = {}
            #statistics_qq_rev[ds.id][exp] = {}
            len_not_interp_model[ds.id][exp] = []
            len_interp_hfr[ds.id][exp] = []
        ...
        minimum_value = np.nanmin([min(mod_values), min(obs_values)])
        mod_values = max_model_rev_value[ds.id].values()
        obs_values = max_obs_rev[ds.id].values()
        maximum_value = np.nanmax([max(mod_values), max(obs_values)])
        ...
        for exp in range(len(name_exp_arr)):
            ...
            for time_counter, index in enumerate(range(idx1, idx2+1)):
                ...
                len_interp_hfr[ds.id] = append_value(
                    len_interp_hfr[ds.id], exp, masked_hfr_speed_interpolated.size)
                ...
                if not isinstance(spatial_mean_hfr_ts_instant, float):
                    spatial_mean_hfr_ts_instant = np.nan
                ...
                len_not_interp_model[ds.id] = append_value(
                    len_not_interp_model[ds.id], exp, masked_subset_speed_model[ds.id][exp][time_counter].size)
                print(
            ...
            plotname = ds.id + '_' + date_in + '_' + date_fin + \
                '_' + time_res_to_average + '_scatter_areal_info_rev_interp.png'
            ...
            if timerange.shape[0] > 2:
                ...
                statistics_rev_interp_areal_info[ds.id][exp] = row_stat_rev_interp
            ...
            a = ((masked_subset_u_model[ds.id][exp] ** 2 +
                  masked_subset_v_model[ds.id][exp] ** 2) ** 0.5).ravel()
            b = ((masked_hfr_u_interpolated_arr[ds.id][exp] ** 2 +
                  masked_hfr_v_interpolated_arr[ds.id][exp] ** 2) ** 0.5).ravel()
            a = a[~np.isnan(b)]
            ...
            plotname = ds.id + '_' + label_plot_arr[exp] + '_' + date_in + '_' + date_fin + \
                '_' + time_res_to_average + '_qqPlot_rev.png'
            ...
            mean_vel_mod = np.mean(a[~np.isnan(a)])
            mean_vel_obs = np.mean(b[~np.isnan(a)])
            tot_mean_stat = [mean_vel_mod, mean_vel_obs]
            row_stat = tot_mean_stat + statistics_array_qq
            statistics_qq_rev[ds.id][exp] = row_stat
            ...
            b = ((masked_subset_u_model[ds.id][exp] ** 2 +
                  masked_subset_v_model[ds.id][exp] ** 2) ** 0.5).ravel()
            mod_array_qq_rev = append_value(mod_array_qq_rev, exp, list(b))
            b = ((masked_hfr_u_interpolated_arr[ds.id][exp] ** 2 +
                  masked_hfr_v_interpolated_arr[ds.id][exp] ** 2) ** 0.5).ravel()
            obs_array_qq_rev = append_value(obs_array_qq_rev, exp, list(b))
        ...
        for exp in range(len(name_exp_arr)):
            ...
            name_file_substring = "mod_averaged_windrose_rev_" + \
                name_exp_arr[exp]
            title_substring = name_exp_arr[exp] + \
                "Model Averaged Windrose for " + ds.id
            a = c_mod_original[ds.id][exp].ravel()
            b = ((masked_subset_u_model[ds.id][exp] ** 2 +
                  masked_subset_v_model[ds.id][exp] ** 2) ** 0.5).ravel()
            plot_windrose(a[~np.isnan(a)], b[~np.isnan(a)], minimum_value, maximum_value, ds, date_in, date_fin, name_file_substring, title_substring, path_to_out_plot_folder_arr[exp], min(
    ...
    for exp in range(len(label_plot_arr)):
        ...
        title = 'Mean Surface Current Velocity -ALL (Rev Interp)\n Period: ' + \
            date_in + '-' + date_fin
        xlabel = 'Observation Current Velocity [m/s]'
        ylabel = 'Model Current Velocity [m/s]'
        if timerange.shape[0] > 2:
            statistics_array_rev_interp = scatterPlot(unlisted_array_mod, unlisted_array_obs, path_to_out_plot_folder_arr[exp] + plotname, label_plot_arr[exp], len(
                listOfFiles)-rem_count, timerange.shape[0], possible_markers, hfr_names_rev, possible_colors, string_time_res, len_not_nan_values=len_not_nan_values_rev[exp], title=title, xlabel=xlabel, ylabel=ylabel)
        row_all_rev_interp = mean_all_rev_interp + statistics_array_rev_interp
        statistics_rev_interp_areal_info["ALL HFR STATIONS"][exp] = row_all_rev_interp

        a_file = open(path_to_out_plot_folder_arr[exp]+"statistics_rev_interp_areal_info_" +
        ...
        print("stat: ", statistics_rev_interp_areal_info)
        for key, value in statistics_rev_interp_areal_info.items():
            array = [key] + value[exp]
            print(array)
            writer.writerow(array)
        a_file.close()

        unlisted_array_mod = np.array(unlist(mod_array_qq_rev[exp]))
        unlisted_array_obs = np.array(unlist(obs_array_qq_rev[exp]))
        tot_mean_mod = round(np.nanmean(unlisted_array_mod), 2)
        tot_mean_obs = round(np.nanmean(unlisted_array_obs), 2)
        mean_all = [tot_mean_mod, tot_mean_obs]

        plotname = date_in + '_' + date_fin + '_' + \
            time_res_to_average + '_qq__rev_info.png'
        title = 'Surface Current Velocity -ALL \n Period: ' + date_in + '-' + date_fin
        xlabel = 'Observation Current Velocity [m/s]'
        ylabel = 'Model Current Velocity [m/s]'
        if timerange.shape[0] > 2:
            statistics_array_qq = QQPlot(unlisted_array_mod, unlisted_array_obs, path_to_out_plot_folder_arr[exp] + plotname,
                                         label_plot_arr[exp], title=title, xlabel=xlabel, ylabel=ylabel)
        row_all = mean_all + statistics_array_qq
        statistics_qq_rev["ALL HFR STATIONS"][exp] = row_all

        a_file = open(path_to_out_plot_folder_arr[exp]+"statistics_qq_rev_plots_" +
                      name_exp_arr[exp] + "_" + date_in + "_" + date_fin + ".csv", "w")
        writer = csv.writer(a_file)
        writer.writerow(["name_hfr", "mean_mod", "mean_obs", "bias",
                        "rmse", "si", "corr", "stderr", "number_of_points"])
        print("statistics: ", statistics_qq_rev)
        for key, value in statistics_qq_rev.items():
            array = [key] + value[exp]
            writer.writerow(array)
        a_file.close()
...
#                continue
#            title_substring = 'Spatial Surface Current Velocity Mean Comparison'
#            name_file_substring = '_mod_obs_ts_comparison_all_rev'

#            plot_mod_obs_ts_comparison_1(spatial_mean_interp_hfr_ts , spatial_not_interp_mean_model_ts , time_res_to_average, ds, date_in, date_fin,
