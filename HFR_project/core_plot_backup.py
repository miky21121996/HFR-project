...
def plot_hfr_wind_field(info, extent, minimum, maximum, x, y, speed_hfr, U, V, skip, skip_coords, date_str, ds, output_plot_folder):
    ...
    plt.figure(num=None, figsize=(18, 13), dpi=100,
    ...
    gl = ax.gridlines(crs=ccrs.PlateCarree(), draw_labels=True, linewidth=0.5,
                      color='gray', alpha=0.5, linestyle='--')  # adding grid lines
    ...
    if info['antennas']:
        for antenna in info['antennas']:
            plt.plot(antenna['lon'], antenna['lat'], color=np.random.rand(3,), markeredgecolor=np.random.rand(
                3,), marker='o', transform=ccrs.Geodetic(), label=antenna['name'])  # add point
    ...
    ax.pcolor(x, y, speed_hfr, cmap='viridis', vmin=minimum, vmax=maximum,
    ...
    ax.quiverkey(Q, 0.8, 1.07, quiver_label, str(quiver_label) +
                 " m/s", fontproperties={'weight': 'bold'}, labelpos='E')

    # title and colorbar
    sm = plt.cm.ScalarMappable(cmap='viridis', norm=norm)
    a = np.random.random((10, 20))
    im_ratio = a.shape[0]/a.shape[1]
    ticks = np.linspace(minimum, maximum, 5, endpoint=True)
    cb = plt.colorbar(sm, ax=ax, orientation='vertical', pad=0.15,
                      fraction=0.047*im_ratio, format=FormatStrFormatter('%.2f'), ticks=ticks)
...
def plot_model_wind_field(info, extent, minimum, maximum, x, y, skip, skip_coords, masked_speed_interpolated, masked_u_interpolated, masked_v_interpolated, date_str, output_plot_folder, label_plot, title_substring, name_file_substring, ds, spatial_mean_model_ts_instant):
    ...
    if info['antennas']:
        for antenna in info['antennas']:
            plt.plot(antenna['lon'], antenna['lat'], color=np.random.rand(3,), markeredgecolor=np.random.rand(
                3,), marker='o', transform=ccrs.Geodetic(), label=antenna['name'])
            if id in list(ds.attrs.keys()):
                if ds.id in ["GL_TV_HF_HFR-TirLig-Total", "GL_TV_HF_HFR-NAdr-Total"]:
                    ax.legend(loc='lower left')
                else:
    ...
    print("max: ", maximum)
    norm = colors.Normalize(vmin=minimum, vmax=maximum)
    ax.pcolor(x, y, masked_speed_interpolated, cmap='viridis', vmin=minimum,
              vmax=maximum, transform=cartopy.crs.PlateCarree())
...
def plot_bias(info, extent, x, y, min_bias, max_bias, masked_speed_interpolated, speed_hfr, date_str, output_plot_folder, label_plot, title_substring, name_file_substring, ds):
    ...
    ax.pcolor(x, y, masked_speed_interpolated-speed_hfr, norm=norm,
              cmap='RdBu_r', transform=cartopy.crs.PlateCarree())
...
def plot_rmsd(info, extent, x, y, min_rmsd, max_rmsd, masked_speed_interpolated, speed_hfr, date_str, output_plot_folder, label_plot, title_substring, name_file_substring, ds):
    ...
    ax.pcolor(x, y, np.sqrt((masked_speed_interpolated-speed_hfr)**2), cmap='viridis',
              vmin=min_rmsd, vmax=max_rmsd, transform=cartopy.crs.PlateCarree())
...
def scatterPlot(mod, obs, outname, name, n_stations, n_time, possible_markers, hfr_name, pos_colors, time_string, **kwargs):
    ...
    im = mscatter(x, y, ax=ax, m=m,
                  c=c_prova[~np.isnan(obs) & ~np.isnan(mod)], s=20)
    marker_array = []
    if n_stations == 1:
        marker_array.append(mlines.Line2D([], [], color='blue', marker=possible_markers[0],
                            linestyle='None', markersize=8, label=markers_labels))
    else:
        for mark, mark_label in zip(possible_markers, markers_labels):
            print('label: ', mark_label)
            marker_array.append(mlines.Line2D(
                [], [], color='blue', marker=mark, linestyle='None', markersize=8, label=mark_label))
    print('im leg ele: ', im.legend_elements(num=n_time))
...
def plot_interpolated_hfr_wind_field(info, extent, minimum, maximum, x_subset_model, y_subset_model, skip, skip_coords, masked_hfr_speed_interpolated, masked_hfr_u_interpolated, masked_hfr_v_interpolated, date_str, output_plot_folder, title_substring, name_file_substring, ds):
    ...
    norm = colors.Normalize(vmin=minimum, vmax=maximum)
    ax.pcolor(x_subset_model, y_subset_model, masked_hfr_speed_interpolated, cmap='viridis', vmin=minimum,
              vmax=maximum, transform=cartopy.crs.PlateCarree())
    # quiver plot: arrows
    Q = ax.quiver(x_subset_model[skip_coords], y_subset_model[skip_coords], masked_hfr_u_interpolated[skip]/maximum,
                  masked_hfr_v_interpolated[skip]/maximum, transform=cartopy.crs.PlateCarree())
    quiver_label = np.round(0.5*(maximum-minimum), 2)
    ax.quiverkey(Q, 0.1, 0.9, quiver_label, str(quiver_label) +
                 " m/s", fontproperties={'weight': 'bold'})
...
def plot_windrose(direction, velocity, minimum, maximum, ds, date_in, date_fin, name_file_substring, title_substring, output_plot_folder, ymin, ymax):
    ...
    print(output_plot_folder + plotname)
